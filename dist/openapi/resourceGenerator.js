 ...
 */
export const schemaNameToUri = (schemaName) => `schema://${toCamelCase(schemaName)}`;
/**
 * Creates a resource URI from an operation.
 * @param operation - API operation
 * @returns Resource URI
 */
export const operationToUri = (operation) => {
 ...
 */
export const determineMimeType = (schema) => {
 ...
 */
export const buildSchemaDescription = (schemaName, schema) => {
 ...
 */
export const schemaToResource = (schemaName, schema) => ({
 ...
 */
export const schemasToResources = (spec) => R.pipe(extractSchemas, R.toPairs, R.map(([name, schema]) => schemaToResource(name, schema)))(spec);
/**
 ...
 */
export const isResourceableOperation = (operation) => {
    if (operation.method !== 'get')
        return false;
    const requiredPathParams = (operation.parameters ?? []).filter((p) => p.in === 'path' && p.required !== false);
 ...
 * @returns Resource description
 */
export const buildOperationDescription = (operation) => {
    const parts = [];
 ...
 * @returns MCP resource definition
 */
export const operationToResource = (operation) => ({
 ...
 * @returns Array of MCP resources
 */
export const operationsToResources = (spec) => R.pipe(extractOperations, R.filter(isResourceableOperation), R.map(operationToResource))(spec);
/**
 * Combines schema and operation resources from a spec.
 * @param spec - OpenAPI specification
 * @returns All MCP resources
 */
export const extractAllResources = (spec) => [
 ...
 */
export const filterResources = (predicate) => (resources) => R.filter(predicate, resources);
/**
 * Filters resources by URI prefix.
 * @param prefix - URI prefix to match
 * @returns A function that filters resources
 */
export const filterByUriPrefix = (prefix) => (resources) => filterResources((r) => r.uri.startsWith(prefix))(resources);
/**
 * Groups resources by their URI scheme.
 * @param resources - Array of resources
 * @returns Resources grouped by scheme
 */
export const groupByScheme = (resources) => R.groupBy((r) => {
 ...
 */
export const createResourceTemplate = (operation) => {
 ...
 */
export const hasUniqueUris = (resources) => {
 ...
 */
export const resolveUriConflicts = (resources) => {
 ...
 */
export const buildResourcesPipeline = R.pipe(extractAllResources, resolveUriConflicts);
